...
def main_run_disptach(pypsa_net, 
    ...
    # Define individual snapshots to formulate
    # one single opf formulation per period
    grouped_snapshots = get_indivitual_snapshots_per_mode(params)

    start = time.time()
    results = []
    for _, snaps in grouped_snapshots:
        # Select indiv load and gen contraints 
        load_2_opf = load_.loc[snaps]
        for k, v in gen_constraints_.items():
            gen_constraints_[k] = v.loc[snaps]
        # Run OPF individual formulations
        results.append(run_opf(pypsa_net, load_2_opf, gen_constraints_, params))
